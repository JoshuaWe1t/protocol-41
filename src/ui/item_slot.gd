...
@onready var charges_label: Label = %Charges # 1. Подключаем ссылку на узел
...
func setup(item_data: Dictionary) -> void:
	...
	# 2. Выставляем значение при инициализации слота
	_update_charges_ui()
...
func _on_item_used(item_number: int) -> void:
	if item_number == item_id: 
		if current_charges > 0 and not Global.item_cooldowns.get(item_id, false):
			...
			# 3. Обновляем надпись сразу после списания заряда
			_update_charges_ui()
...
func _update_charges_ui() -> void:
	charges_label.text = "%d/%d" % [current_charges, max_charges]
